...
/**
 * Initialize Memory Strategy in Serena MCP
 *
 * This script runs on SessionStart and ensures that the memory strategy
 * document exists in Serena MCP memory storage.
 *
 * Purpose:
 * - Activate Serena MCP project
 * - Check if memory-strategy.md exists
 * - Create it if missing with the core memory strategy principles
 */
...
// Memory strategy content based on report03.md
const MEMORY_STRATEGY_CONTENT = `# Memory Strategy for AI Agent
type: policy
confidence: high
project: global
expires: none
...
function log(message) {
  console.log(`[init-memory-strategy] ${message}`);
}
...
function checkMemoryStrategyExists() {
  return fs.existsSync(MEMORY_STRATEGY_FILE);
}

function createMemoryStrategy() {
  log(`Creating memory strategy: ${MEMORY_STRATEGY_FILE}`);
  fs.writeFileSync(MEMORY_STRATEGY_FILE, MEMORY_STRATEGY_CONTENT, 'utf8');
  log('Memory strategy created successfully');
}

function main() {
  try {
    log('Starting memory strategy initialization...');

    // Ensure .serena/memories directory exists
    ensureDirectoryExists(MEMORY_DIR);

    // Check if memory strategy already exists
    if (checkMemoryStrategyExists()) {
      log('Memory strategy already exists, skipping creation');
      return;
    }
    ...
    log('Memory strategy initialization complete');
  } catch (error) {
    console.error(`[init-memory-strategy] Error: ${error.message}`);
    // Don't fail the session start if this fails
    process.exit(0);
  }
}
