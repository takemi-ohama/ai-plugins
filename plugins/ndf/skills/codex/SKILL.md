---
name: codex
description: "codex CLI (OpenAI Codex) を直接実行してコード生成・レビュー・調査を外部AIに委譲する手順。MCP経由ではなく `codex exec` をバックグラウンド実行する。サンドボックス制約の回避、stdin/stderr経路、バックグラウンド待機パターンを扱う。Triggers: 'codexで調査', 'codexレビュー', '第二意見レビュー', 'codex exec', 'external AI review'"
---
...
`codex` CLI（OpenAI Codex、通常は `/usr/bin/codex` または `npm` 経由でインストール）を **MCP経由ではなく直接実行**して、コード生成・独立レビュー・コードベース調査を外部AIに委譲するためのスキル。

MCP版の codex ツールは **ホスト側のファイルシステムにアクセスできない**制約があるため、ローカルファイルの逐語照合レビューや大規模コードベース調査には **CLI直接実行が必須**。

## NDFとの関係

- NDFプラグインの `corder` エージェントはCodex MCPを使った軽量な「第二意見レビュー」を担当
- 本スキルは「MCPでは不十分な場面」（ファイル逐語照合、大規模コードベース走査、長時間実行）で **CLI 直接実行**に切り替える手順をまとめる
- `/ndf:corder` と併用するケース: corderで概要レビュー → 深掘りが必要なら codex CLI 直接実行で逐語照合
...
NDFプラグインに含まれるCodex MCP（`corder` エージェント経由）との使い分け:

| 観点 | MCP版（corder） | CLI直接実行 |
|---|---|---|
| ホスト側ファイル読み込み | ❌ 制約あり | ✅ 絶対パスで読める |
| 行番号照合 | ❌ 不正確 | ✅ 正確 |
| 大規模コードベース調査 | ❌ 情報不足 | ✅ 適切 |
| 短い設計相談 | ✅ 軽量 | △ オーバーヘッド |
| バックグラウンド実行 | ❌ 同期のみが多い | ✅ `&` で非同期化可 |
| エージェント側の待機 | 必須（数分） | スケジュール機能で他作業と並列 |

**ルール**: 「ファイルを逐語的に読む必要があるか」で判断。YES → CLI直接実行、NO → corder で軽量に済ませる。
...
- **NDF `corder` エージェント**: Codex MCPによる軽量な第二意見レビュー
